...
const ExamRoadmap = () => {
     ...
     useEffect(() => {
          ...
          } else if (TEMPLATES.Corporate && TEMPLATES.Corporate["TCS NQT (Ninja & Digital)"]) {
     ...
     const loadTemplate = (category, templateName) => {
          if (window.confirm(`Load ${templateName} template? This clears current canvas.`)) {
     ...
     const handleSave = async () => {
          const payload = { title: roadmapTitle, nodes, edges, isPublic };
          try {
               const BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:5000/api";
               const response = await axios.post(`${BASE_URL}/roadmaps/save`, payload);

               if (response.data.success) {
                    alert(`Saved! ${isPublic ? "Visible to community." : "Private."}`);
               ...
               <div className="w-80 h-full bg-white border-r border-slate-200 p-6 flex flex-col gap-5 overflow-y-auto shadow-md z-10 custom-scrollbar">
                    ...
                    {isAdmin && (
                         <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-4">
                              <h3 className="text-xs font-bold text-indigo-800 mb-3 uppercase">Admin Database Controls</h3>
                              <input
                                   type="text"
                                   value={roadmapTitle}
                                   onChange={(e) => setRoadmapTitle(e.target.value)}
                                   className="w-full text-lg font-bold text-slate-800 mb-3 border-b-2 border-indigo-200 bg-transparent outline-none"
                              ...
                              <button onClick={handleSave} className="w-full bg-indigo-600 text-white py-2.5 rounded-lg text-sm font-bold">
                                   Save to Database
                              </button>
                         </div>
                    )}
                    ...
                    {isAdmin && (
                         <div className="mb-4">
                              ...
                              <input
                                   ...
                                   className="w-full p-2 text-sm border rounded-lg mb-2 outline-none"
                                   onKeyDown={(e) => e.key === 'Enter' && handleAddCustomNode(false)}
                              />
                              <div className="flex gap-2">
                                   <button onClick={() => handleAddCustomNode(false)} className="flex-1 bg-white border border-slate-300 py-1.5 text-xs font-bold rounded-md">Add Step</button>
                                   <button onClick={() => handleAddCustomNode(true)} className="flex-1 bg-emerald-50 text-emerald-700 border border-emerald-200 py-1.5 text-xs font-bold rounded-md">Add Target</button>
                              </div>
                              ...
                              <div className="flex flex-col gap-2">
                                   {Object.keys(TEMPLATES[category]).map((templateName) => (
                                        <button
                                             ...
                                             className="text-left px-3 py-2 text-sm bg-slate-800 text-slate-100 font-medium rounded-lg hover:bg-slate-700"
                                        >
                                             {templateName}
                                        </button>
                                   ))}
